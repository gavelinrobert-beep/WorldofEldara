...
3. If you're behind a home router, add a **port forwarding** rule for that TCP port → your machine's local IP (find it via `ipconfig` on Windows, `ip addr show` on Linux, or `ifconfig`/`ipconfig getifaddr en0` on macOS).
