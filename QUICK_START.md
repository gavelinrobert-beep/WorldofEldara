...
2. Allow inbound TCP traffic on that port in your OS firewall.
   ...
   - Linux: open the same TCP port with your distro firewall.
     - Ubuntu/Debian: `sudo ufw allow 7777/tcp`
     - Other distros: `sudo iptables -A INPUT -p tcp --dport 7777 -j ACCEPT` (replace 7777 if you changed the port; persist with your distro's iptables-save/netfilter-persistent tools).
...
5. Verify from another device using `telnet 203.0.113.45 7777` (replace with your public IP/port) or the sample client above (point it at your public IP instead of `localhost`) to confirm the server is reachable.
