...
public class Ability
{
    ...
    [Key(4)]
    public MagicSource MagicSource { get; set; }
    ...
    [Key(8)]
    public float Radius { get; set; } // for AoE abilities

    [Key(9)]
    public int ManaCost { get; set; }

    [Key(10)]
    public float CastTime { get; set; } // seconds, 0 = instant

    [Key(11)]
    public float Cooldown { get; set; } // seconds

    [Key(12)]
    public float GlobalCooldown { get; set; } = 1.5f; // seconds
    ...
    [Key(17)]
    public float PowerScaling { get; set; } // multiplier for attack/spell power

    // Status effects
    [Key(18)]
    public List<StatusEffectData> AppliesEffects { get; set; } = new();
    ...
    [Key(20)]
    public List<Class> AllowedClasses { get; set; } = new();

    // Lore justification (for design documentation)
    [Key(21)]
    public string LoreJustification { get; set; } = string.Empty;
    ...
    public int CalculateValue(CharacterStats casterStats, bool isCrit)
    {
        ...
        float value = BaseDamage + (power * PowerScaling);

        if (isCrit && CanCrit)
        {
            value *= casterStats.CriticalDamage;
        }
...
public class StatusEffectData
{
    ...
    [Key(3)]
    public float Duration { get; set; } // seconds, -1 = permanent until removed
    ...
    [Key(6)]
    public bool IsBuff { get; set; } // true = buff, false = debuff
    ...
    // Periodic effects
    [Key(8)]
    public int PeriodicValue { get; set; } // damage/healing per tick

    [Key(9)]
    public float TickInterval { get; set; } // seconds between ticks
