...
public static class MovementPackets
{
    ...
    {
        [Key(0)] public uint InputSequence { get; set; } // Client-side input sequence number

        [Key(1)] public float DeltaTime { get; set; }

        [Key(2)] public MovementInput Input { get; set; } = new();

        [Key(3)] public Vector3 PredictedPosition { get; set; } // Client's predicted position

        [Key(4)] public float PredictedRotationYaw { get; set; }
    }
    ...
    public class MovementUpdatePacket : PacketBase
    {
        ...
        [Key(3)] public float RotationYaw { get; set; }

        [Key(4)] public float RotationPitch { get; set; }

        [Key(5)] public MovementState State { get; set; }

        [Key(6)] public long ServerTimestamp { get; set; }
    }
    ...
    {
        [Key(0)] public uint LastProcessedInput { get; set; } // Which input sequence this correction is for

        [Key(1)] public Vector3 AuthoritativePosition { get; set; }

        [Key(2)] public Vector3 AuthoritativeVelocity { get; set; }
...
{
    [Key(0)] public float Forward { get; set; } // -1 to 1

    [Key(1)] public float Strafe { get; set; } // -1 to 1

    [Key(2)] public bool Jump { get; set; }

    [Key(3)] public bool Sprint { get; set; }

    [Key(4)] public float LookYaw { get; set; } // Camera rotation

    [Key(5)] public float LookPitch { get; set; }
}
