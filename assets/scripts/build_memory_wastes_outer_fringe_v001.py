...
STEM = "memory_wastes_outer_fringe_v001"
...
def build_rootshear_rift(collections, mats):
    ...
    make_ground_patch("MW_ROOTSHEAR_RIFT_MainCrackedGroundMound", col, (5.5, 18.0, 0.33), 12.5, 8.7, mats["MAT_MWOF_Ground_MemoryMud"], rot=-6, height=0.26, sides=13)
    disc_mesh("MW_ROOTSHEAR_RIFT_CyanMemoryLeak_GroundFissureGlow", col, (5.5, 18.0, 0.58), 6.2, 1.15, mats["MAT_MWOF_Worldroot_Cyan"], sides=34, rot=(0, 0, math.radians(7)))
    cube_obj("MW_ROOTSHEAR_RIFT_DarkSplitMouth_InPhysicalTerrain", col, (5.5, 18.0, 0.72), (8.8, 0.42, 0.12), mats["MAT_MWOF_Path_CrackedDark"], rot=(0, math.radians(2), math.radians(7)))
    ...
    for i, (dx, dy, z, mat_key) in enumerate([(-1.8, -0.2, 2.2, "MAT_MWOF_Worldroot_Cyan"), (0.8, 0.4, 2.8, "MAT_MWOF_Violet_Instability"), (2.5, -0.6, 2.0, "MAT_MWOF_Worldroot_Cyan_Dim"), (-3.0, 1.1, 1.8, "MAT_MWOF_Worldroot_Cyan_Dim")]):
        bicone_mesh(f"MW_ROOTSHEAR_RIFT_FloatingMemoryShard_Controlled_{i:02d}", col, (5.5 + dx, 18.0 + dy, z), 0.16, 0.55, mats[mat_key], sides=5, rot=(math.radians(8), math.radians(0), math.radians(i * 28)))
    cube_obj("MW_ROOTSHEAR_RIFT_LowVerticalCyanMemorySteam_Plane", col, (5.5, 18.2, 2.2), (0.10, 0.10, 3.2), mats["MAT_MWOF_Worldroot_Cyan"], rot=(math.radians(2), math.radians(0), math.radians(7)))
...
def write_report(manifest):
    report = f"""# Memory Wastes - Outer Fringe v001 Report
...
## What Changed
- Re-scoped Memory Wastes from full floating archipelago/core-collapse benchmark into the first grounded playable edge of the zone.
- Removed the endgame dead-god/floating-void archipelago direction from this output.
- Built an approximately 80m x 60m grounded forest-edge space with a readable broken mossy root-stone path.
- Added Warden containment camp, Echo Flats, Oranyn ruins, Rootshear Rift, memory ghosts, strained forest edge and restrained floating memory fragments.
- Color use is controlled: cyan for Worldroot memory, violet for dangerous rift leakage, pale white-green for Oranyn ruins, warm green/gold for Warden camp.

## Required Answers
1. Did you re-scope the zone from full Memory Wastes to Outer Fringe?
   Yes. This scene is now Memory Wastes - Outer Fringe: a grounded first playable edge where memory overflow has started, not the full endgame collapse.

2. Is the scene mostly grounded rather than floating?
   Yes. The scene uses one continuous grounded terrain mass, banks, path slabs, forest edge mounds and localized rift debris. Floating pieces are limited to low memory fragments and a few controlled shards near the rift/ruins.

3. Is the main path readable?
   Yes. A broken mossy root-stone path starts at the player/Warden camp foreground and curves toward the Rootshear Rift, with root borders and only subtle cyan route runes.

4. Is the Rootshear Rift the main landmark?
   Yes. MW_ROOTSHEAR_RIFT is the strongest far-mid focal point, using cracked terrain, exposed root fibers, cyan memory light, violet leakage and a few floating memory shards.

5. Do the Oranyn ruins read clearly?
   Yes. MW_ORANYN_RUINS uses a large broken arch, partial columns, incomplete stairs, low floating wall fragments, pale white-green stone and ghost-cyan cracks.

6. Does the zone feel like early memory overflow rather than endgame collapse?
   Yes. The zone is still grounded and traversable: broken forest edge, containment camp, small memory leak pools, restrained ruins and a medium-scale rift rather than a vast void apocalypse.

7. What still reads as placeholder?
   Player/NPC/ghost silhouettes are intentionally placeholder. Terrain and path are visual benchmark meshes rather than final collision-ready gameplay terrain. Materials are controlled stylized color direction, not final hand-painted texture assets.
